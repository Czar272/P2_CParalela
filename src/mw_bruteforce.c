...
static int read_file(const char* path, unsigned char** buf, size_t* len) {
  ...
  *buf = (unsigned char*)malloc((size_t)L);
  ...
  if (fread(*buf, 1, (size_t)L, f) != (size_t)L) { fclose(f); free(*buf); return -4; }
  fclose(f);
  *len = (size_t)L;
  return 0;
}
...
static u64 pick_stride(u64 len, u64 seed) {
  ...
  for (int k = 0; k < 64; ++k) {
    ...
    if (ugcd(cand, len) == 1) return cand;
  }
...
static int contains_subsequence(const unsigned char *hay, size_t haylen,
  ...
  for (size_t i = 0; i + needlen <= haylen; ++i) {
    if (hay[i] == (unsigned char)needle[0]) {
      size_t j = 1;
      for (; j < needlen; ++j)
        if (hay[i + j] != (unsigned char)needle[j]) break;
...
static int search_chunk_random(u64 start, u64 len, u64 seed,
                               const unsigned char* cipher, size_t clen,
                               const char* keyword,
                               u64* found_key, int rank) {
  if (len == 0) return 0;

  // Elegimos un “stride” coprimo con len y un desplazamiento a partir de la semilla
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    if (!strcmp(argv[i], "--cipher") && i+1 < argc) cipher_path = argv[++i];
      ...
      if (rank==0) {
        fprintf(stderr,
          "Uso: mpirun -np P ./mw_bruteforce --cipher FILE --keyword \"...\" --start A --end B [--chunk N]\n");
      }
      MPI_Finalize(); return 0;
    }
  }
  if (rank == 0) {
    printf("[DEBUG master] cipher_path=%s keyword=%s start=%" PRIu64 " end=%" PRIu64 " chunk=%" PRIu64 "\n",
  ...
  if (rank == 0) {
    if (read_file(cipher_path, &cipher, &clen) != 0) die("No se pudo leer el archivo cifrado.");
  }
  ...
  printf("[DEBUG worker %d] after recv cipher first_byte=0x%02x len=%zu\n", rank,
         clen>0 ? (unsigned int)cipher[0] : 0, clen); fflush(stdout);
}
  ...
  if (rank == 0) {
    ...
    while (live > 0) {
      MPI_Status st;
      int fflag = 0; MPI_Iprobe(MPI_ANY_SOURCE, TAG_FOUND, MPI_COMM_WORLD, &fflag, &st);
      if (fflag) {
        u64 k; MPI_Recv(&k, 1, MPI_UNSIGNED_LONG_LONG, st.MPI_SOURCE, TAG_FOUND, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        int req_rank = st.MPI_SOURCE;
        printf("[DEBUG master] pedido de trabajo recibido de %d\n", req_rank); fflush(stdout);

        found_flag = 1; winner = k;
      ...
      MPI_Recv(&req_byte, 1, MPI_UNSIGNED_CHAR, MPI_ANY_SOURCE, TAG_REQ, MPI_COMM_WORLD, &st);
      req_rank = st.MPI_SOURCE;

      // Si ya hubo ganador, manda STOP
      if (found_flag) {
        MPI_Send(&winner, 1, MPI_UNSIGNED_LONG_LONG, req_rank, TAG_STOP, MPI_COMM_WORLD);
        continue;
      }
      ...
      if (next >= total) {
        ...
        u64 len = chunk_size;
        if (len > total - next) len = total - next;
        u64 start = key_start + next;
        u64 seed  = splitmix64(base_seed ^ start);
        u64 triple[3] = {start, len, seed};
        MPI_Send(triple, 3, MPI_UNSIGNED_LONG_LONG, req_rank, TAG_WORK, MPI_COMM_WORLD);
        next += len;
    ...
    // Si terminamos sin ganador, manda STOP con ULLONG_MAX para cerrar a los que sigan vivos
    if (!found_flag) {
      winner = UINT64_MAX;
      for (int r = 1; r < world; ++r) {
        MPI_Send(&winner, 1, MPI_UNSIGNED_LONG_LONG, r, TAG_STOP, MPI_COMM_WORLD);
    ...
    for (;;) {
      ...
      if (st.MPI_TAG == TAG_WORK) {
        ...
        printf("[DEBUG worker %d] recv work start=%" PRIu64 " len=%" PRIu64 " seed=%" PRIu64 "\n",
         rank, start, len, seed); fflush(stdout);

        // Recorre el chunk en orden pseudoaleatorio
        u64 kfound = 0;
        int ok = search_chunk_random(start, len, seed, cipher, clen, keyword, &kfound, rank);
        if (ok) {
          // Reporta a master y termina
          MPI_Send(&kfound, 1, MPI_UNSIGNED_LONG_LONG, 0, TAG_FOUND, MPI_COMM_WORLD);
        ...
        int sf = 0; MPI_Iprobe(0, TAG_STOP, MPI_COMM_WORLD, &sf, &st);
        if (sf) {
          MPI_Recv(&winner, 1, MPI_UNSIGNED_LONG_LONG, 0, TAG_STOP, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
          found_flag = (winner != UINT64_MAX);
          break;
