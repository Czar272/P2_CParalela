...
int main(int argc, char **argv) {
    ...
    if (rank == 0) {
        ...
        if (winner >= 0) {
            ...
            uint64_to_deskey(all_results[winner].key, key);
            memcpy(workbuf, cipher, cipher_len);
            des_decrypt_buffer(workbuf, cipher_len, key);
